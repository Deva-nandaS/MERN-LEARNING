    ...
    <div className="overflow-x-auto">
      <table className="min-w-[900px] w-full text-left border-collapse text-sm">
        <thead className="bg-gray-50 text-gray-600 text-xs uppercase">
          <tr>
            {["NAME","TYPE","AUTH TYPE","STATUS","UPDATED BY","UPDATED AT","ACTIONS"].map((h) => (
              <th key={h} className="p-4 border border-gray-300">{h}</th>
            ))}
          </tr>
        </thead>
        ...
        <tbody>
          {sources.map((item) => (
            <tr key={item._id} className="hover:bg-gray-50">
              ...
              <td className="p-4 border border-gray-300">{item.authType || "N/A"}</td>
          ...
          <tr>
            <td colSpan="7" className="p-4 border-t border-gray-300 text-gray-500">
              <div className="flex flex-wrap gap-2 items-center">
                <span>Showing 1 to {sources.length} of {sources.length} results</span>
                <span>Show:</span>
